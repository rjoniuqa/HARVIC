...
public class ContourPersonDetector implements PersonDetector {
	...
	public MatOfRect detect(Mat frame) {
		...
		List<MatOfPoint> contourPoints = new ArrayList<MatOfPoint>();
		findContours(fgMask, contourPoints, new Mat(), Imgproc.RETR_EXTERNAL,
			Imgproc.CHAIN_APPROX_SIMPLE);
		...
		for (MatOfPoint point : contourPoints) {
			...
			Rect rect = boundingRect(point);
			CustomRect cRect = new CustomRect(rect.x, rect.y, rect.width,
				rect.height);
	...
	private List<Rect> groupRectangles(List<CustomRect> rectangles,
		int threshold) {
		...
		if (clusters.size() > 1) {
			...
			do {
				...
				Object[] currentDistanceResult = clusters.get(0)
					.findNearest(clusters);
	...
	private class CustomRect extends Rect {
		...
		public Object[] findNearest(List<CustomRect> rectangles) {
			...
			for (int i = 1; i < rectanglesCopy.size(); i++) {
				...
				if ((currentDistance = distanceTo(
					rectanglesCopy.get(i))) < lowestDistance) {
		...
		public double distanceTo(CustomRect cluster) {
			...
			Rectangle other = new Rectangle(cluster.x, cluster.y, cluster.width,
				cluster.height);
			...
			if (othersLeftBoundary > selfRightBoundary) {
				if (othersTopBoundary <= selfBottomBoundary
					&& othersBottomBoundary >= selfBottomBoundary
					|| othersTopBoundary <= selfTopBoundary
						&& othersBottomBoundary >= selfTopBoundary) {
					ay = by = 0;
				} else {
					if (othersTopBoundary > selfBottomBoundary) {
				...
				if (othersLeftBoundary <= selfRightBoundary
					&& othersRightBoundary >= selfRightBoundary
					|| othersLeftBoundary <= selfLeftBoundary
						&& othersRightBoundary >= selfLeftBoundary) {
					ax = bx = 0;
				} else {
